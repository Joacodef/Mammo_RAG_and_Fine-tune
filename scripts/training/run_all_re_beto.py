...
def run_regime(regime_name, train_file_name):
    print(f"\n==================== Starting BETO RE Training/Evaluation for: {regime_name} ====================")
    
    # 1. Train BETO models for the 5 folds for RE
    run_cmd([
        sys.executable, "scripts/training/run_re_training.py",
        "--config-path", "configs/training_re_config.yaml",
        "--partition-dir", "data/processed/train-50",
        "--train-file-name", train_file_name
    ])
    
    # Get the latest run output folder in output/models/re
    model_dir = get_latest_dir("output/models/re/*")
    print(f"Detected model output directory: {model_dir}")
    if not model_dir:
        raise FileNotFoundError("Could not find newly trained model folder in output/models/re")
        
    run_folder_name = os.path.basename(model_dir)
    
    # 2. Generate predictions on test set for all 5 folds
    print(f"\nGenerating predictions for BETO RE - {regime_name}...")
    run_cmd([
        sys.executable, "scripts/evaluation/generate_finetuned_predictions.py",
        "--config-path", "configs/inference_re_config.yaml",
        "--model-dir", model_dir
    ])
    
    # 3. Calculate final aggregate metrics
    print(f"\nCalculating aggregate metrics for BETO RE - {regime_name}...")
    pred_dir = os.path.join("output/finetuned_results/re", run_folder_name)
    run_cmd([
    ...
    print(f"==================== Completed BETO RE for: {regime_name} ====================\n")
...
def main():
    ...
    print(f"\n==================== All BETO RE training and evaluations completed in {elapsed:.2f} minutes! ====================")
